...
import { VisualTemplate, VisualStyle, coordKey, BlockAspect } from '../types/visual';
...
export const FormatStylePanel: React.FC<FormatStylePanelProps> = ({
  ...
  const k = selectedCoord
    ? coordKey(selectedCoord.row, selectedCoord.col)
    : undefined;
  const current: VisualStyle = k ? visualTemplate[k] ?? {} : {};
  ...
  return (
    <div className="format-style-panel">
      ...
      {/* Relación de aspecto del bloque */}
      <div className="field">
        <label>📐 Relación de aspecto</label>
        <select
          value={blockAspect}
          onChange={(e) => onUpdateAspect(e.target.value as BlockAspect)}
        >
          <option value="1/1">1:1</option>
          <option value="1/2">1:2 (vertical)</option>
          <option value="2/1">2:1 (apaisado)</option>
        </select>
      </div>

      {!selectedCoord && <p>Selecciona una celda activa para editar su formato.</p>}
        ...
        <>
          ...
          <div className="field">
            ...
            <select
              ...
              onChange={(e) =>
                patch({ textAlign: e.target.value as VisualStyle['textAlign'] })
              }
            >
          ...
          <div className="field">
            ...
            <div className="control-row">
              ...
              <input
                ...
                value={fontPx}
                onChange={(e) =>
                  patch({ fontSizePx: clamp(Number(e.target.value), 10, 36) })
                }
                style={{ flex: 1 }}
              />
              <button
          ...
          <div className="field">
            ...
            <div className="control-row">
              ...
              <input
                ...
                value={padX}
                onChange={(e) =>
                  patch({ paddingX: clamp(Number(e.target.value), 0, 32) })
                }
                style={{ flex: 1 }}
              />
              <button
          ...
          <div className="field">
            ...
            <div className="control-row">
              ...
              <input
                ...
                value={padY}
                onChange={(e) =>
                  patch({ paddingY: clamp(Number(e.target.value), 0, 32) })
                }
                style={{ flex: 1 }}
              />
              <button
